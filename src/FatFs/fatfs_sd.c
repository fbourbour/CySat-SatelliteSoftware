...
{
	////vConsolePrintf("xchg_spi: inside");
...
{
	//vConsolePrintf("rcvr_spi_multi: inside");
	
	TM_SPI_ReadMulti(FATFS_SPI, buff, 0xFF, btr);
	
	//vConsolePrintf("rcvr_spi_multi: done");
}
...
{
	//vConsolePrintf("xmit_spi_multi: inside");
...
)
{
	...
	if (d == 0xFF) {
		//vConsolePrintf("wait_ready: OK");
	} else {
		//vConsolePrintf("wait_ready: timeout");
	}
	return (d == 0xFF) ? 1 : 0;
}
...
static int select (void)	/* 1:OK, 0:Timeout */
{
	...
	if (wait_ready(500)) {
		////vConsolePrintf("select: OK");
		return 1;	/* OK */
	}
	//vConsolePrintf("select: no");
	deselect();
	return 0;	/* Timeout */
}
...
)
{
	...
	////vConsolePrintf("rcvr_datablock: inside");
	...
	if (token != 0xFE) {
		////vConsolePrintf("rcvr_datablock: token != 0xFE");
		return 0;		// Function fails if invalid DataStart token or timeout 
	}

	rcvr_spi_multi(buff, btr);		// Store trailing data to the buffer 
	xchg_spi(0xFF); xchg_spi(0xFF);			// Discard CRC 
	////vConsolePrintf("rcvr_datablock: return = 1");
...
)
{
	...
	////vConsolePrintf("xmit_datablock: inside");

	if (!wait_ready(500)) {
		////vConsolePrintf("xmit_datablock: not ready");
		return 0;		/* Wait for card ready */
	}
	//vConsolePrintf("xmit_datablock: ready");
...
)
{
	...
	////vConsolePrintf("send_cmd: inside");
	
	if (cmd & 0x80) {	/* Send a CMD55 prior to ACMD<n> */
		////vConsolePrintf("send_cmd: 0x80 bit set");
		cmd &= 0x7F;
	...
	if (cmd != CMD12) {
		////vConsolePrintf("send_cmd: cmd != CMD12");
		deselect();
	...
	if (cmd == CMD12) {
		//vConsolePrintf("send_cmd: CMD12, receive command resp");
...
void TM_FATFS_InitPins(void) {
	...
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz; //was GPIO_Speed_100MHz;
...
DSTATUS TM_FATFS_SD_disk_initialize (void) {
	...
	//vConsolePrintf("disk_initialize: inside");
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		//vConsolePrintf("disk_initialize: CMD0 = 1");
	...
	//vConsolePrintf("disk_initialize: deselecting");
	deselect();
...
DSTATUS TM_FATFS_SD_disk_status (void) {
	//vConsolePrintf("fatfs_sd_disk_status");
	
	if (!TM_FATFS_Detect()) {
		//vConsolePrintf("not detected");
		return STA_NOINIT;
	}
...
{
	//vConsolePrintf("disk_read: inside");
...
{
	////vConsolePrintf("disk_write: inside");
	if (!TM_FATFS_Detect()) {
		return RES_ERROR;
	}
	if (!TM_FATFS_WriteEnabled()) {
		////vConsolePrintf("disk_write: Write protected!!! \n---------------------------------------------");
		return RES_WRPRT;
	}
